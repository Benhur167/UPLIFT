              ...
              <div key={i} className="card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="font-semibold text-blue-800 flex items-center gap-2">
                    {w.title}
                    <span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded font-normal">{w.duration}</span>
                    {isSignedUp && (
                      <span className="text-[9px] bg-emerald-50 text-emerald-700 font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5 border border-emerald-100 animate-fade-in">
                        ✓ Registered
                      </span>
                    )}
                  </div>
                  <div className="text-sm text-slate-500 mt-1">{w.date}</div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 transition-all duration-300">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <div>
                <h1 className="text-base font-black text-slate-800 leading-tight">{activeWorkshop.title}</h1>
                <div className="flex gap-2 mt-1.5 flex-wrap">
                  <span className="text-[10px] text-slate-500 bg-slate-100 px-2 py-0.5 rounded font-semibold">{activeWorkshop.duration}</span>
                  <span className="text-[10px] text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded font-semibold">{activeWorkshop.date}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 transition-all duration-300">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 text-center space-y-3">
              <div className="w-12 h-12 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center text-xl mx-auto shadow-inner border border-emerald-100">
                ✓
              </div>
              <h3 className="text-sm font-bold text-slate-800">Registration Confirmed!</h3>
